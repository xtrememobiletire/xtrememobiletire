...
import { FaUser, FaCar, FaPhone, FaEnvelope, FaSignOutAlt, FaCheckCircle, FaClock, FaTimesCircle, FaRuler } from 'react-icons/fa';
import API from '../api';
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <p className="text-gray-400 text-xs uppercase tracking-widest">Member Portal</p>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-10 space-y-6">
        ...
        <div className={`flex items-center gap-3 border rounded-xl px-5 py-4 ${s.bg}`}>
          {s.icon}
          <div>
            <p className="text-gray-400 text-xs uppercase tracking-wide">Membership Status</p>
            <p className={`font-bold text-lg ${s.color}`}>{s.label}</p>
          </div>
          {user.status === 'pending' && (
            <p className="ml-auto text-gray-500 text-sm">Your membership is under review.</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div className="bg-[#111] border border-gray-800 rounded-xl p-5 text-center">
            ...
            <p className="text-white text-sm font-bold truncate">{user.vehicle}</p>
            <p className="text-gray-500 text-xs uppercase tracking-wide mt-1">Vehicle</p>
          </div>
          <div className="bg-[#111] border border-gray-800 rounded-xl p-5 text-center">
            <FaCheckCircle className="text-red-500 text-2xl mx-auto mb-2" />
            <p className="text-white text-lg font-bold">Priority</p>
            <p className="text-gray-500 text-xs uppercase tracking-wide mt-1">Service</p>
          </div>
          <div className="bg-[#111] border border-gray-800 rounded-xl p-5 text-center col-span-2 sm:col-span-1">
            <FaRuler className="text-red-500 text-2xl mx-auto mb-2" />
            <p className="text-white text-sm font-bold">{user.tireSize || 'Not set'}</p>
            <p className="text-gray-500 text-xs uppercase tracking-wide mt-1">Tire Size</p>
          </div>
        </div>

        {/* Member Details */}
        <div className="bg-[#111] border border-gray-800 rounded-2xl p-6">
          <h2 className="text-white font-bold text-lg mb-5">Member Details</h2>
